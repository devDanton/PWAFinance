...
export default async function SubscriptionsPage() {
  ...
  const { data: subscriptions } = await supabase.from('subscriptions').select('*, workspaces(name), credit_cards(name), categories(name, color)').order('next_date', { ascending: true })
  const { data: transactions } = await supabase.from('transactions').select('amount, description, type, is_paid')
